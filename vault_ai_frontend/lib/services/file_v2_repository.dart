...
class FileV2Repository {
  ...
  static FileV2Repository? current() {
    ...
    return _active ??= FileV2Repository._(mvk);
  }
  ...
  Future<SecretKey> _fileKey(String fileId) async {
    ...
    final files = await Hkdf(hmac: Hmac.sha256(), outputLength: 32).deriveKey(
      secretKey: SecretKey(mvkBytes), nonce: const [],
    ...
    return Hkdf(hmac: Hmac.sha256(), outputLength: 32).deriveKey(
      ...
      nonce: utf8.encode(fileId), info: utf8.encode('vaultai.file.record.v2'),
    );
  }
  ...
  Future<Uint8List> encryptEnvelope({required String fileId,
      required Uint8List bytes, required String filename,
      String? contentType, String? relativePath, String? label}) async {
    final payload = jsonEncode({'filename': filename, 'content_type': contentType,
      'relative_path': relativePath, 'label': label, 'bytes': base64Url.encode(bytes)});
    return aesGcmWrapWithAad(await _fileKey(fileId), utf8.encode(payload), aad: _aad(fileId));
  }

  Future<Uint8List> encryptMetadata({required String fileId,
      required String filename, String? contentType, String? relativePath,
      String? label}) async {
    final payload = jsonEncode({'filename': filename, 'content_type': contentType,
      'relative_path': relativePath, 'label': label});
    return aesGcmWrapWithAad(await _fileKey(fileId), utf8.encode(payload), aad: _aad(fileId));
  }

  Future<({Uint8List bytes, String filename, String? contentType, String? relativePath, String? label})>
    ...
    return (bytes: Uint8List.fromList(base64Url.decode(map['bytes'] as String)),
      filename: map['filename'] as String, contentType: map['content_type'] as String?,
      relativePath: map['relative_path'] as String?, label: map['label'] as String?);
  }

  Future<Uint8List> encryptChunk(String fileId, int index, Uint8List bytes) async =>
      aesGcmWrapWithAad(await _fileKey(fileId), bytes, aad: _aad(fileId, index));
