  ...
  group('Crypto Wallet Engine — slice 6 wallet-style redesign', () {
    testWidgets('R1: portfolio summary banner renders — the trailing '
      ...
      expect(
        ...
        findsNothing,
        reason: 'the "VaultAI only shows real on-chain balances" line '
    ...
    testWidgets(
        'R14: Ask VaultAI section renders the closed-set suggested prompts',
        (tester) async {
      tester.view.physicalSize = const Size(1200, 4000);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
      ...
      for (final prompt in kCryptoWalletEngineAskAiSuggestedPrompts) {
        expect(
          find.text(prompt),
          findsOneWidget,
          reason: 'Ask VaultAI prompt missing: $prompt',
        );
      }
    });
    ...
    testWidgets('R19: tapping an Ask VaultAI prompt forwards through callback',
        (tester) async {
      tester.view.physicalSize = const Size(1200, 4000);
      tester.view.devicePixelRatio = 1.0;
      addTearDown(tester.view.resetPhysicalSize);
