...
void main() {
  group('ZK memory-proposal sentinel — string-strip semantics', () {
    ...
    test('opening marker only (mid-chunk) → everything from the '
    ...
    test('full sentinel → payload extracted, sentinel + blank line '
        'stripped from the visible buffer', () {
      final json =
          '{"memory_type":"identity","memory_key":"name",'
          '"memory_value":"Ada","memory_event_date":null}';
      final buf =
          '$kMemoryProposalOpen$json$kMemoryProposalClose\n\n'
          'Got it.\n\nIdentity: Ada';
      final r = extractAndStripMemoryProposal(
        buffer: buf,
        alreadyFinalized: false,
    ...
    test('alreadyFinalized=true → jsonPayload is null but buffer is '
        'still stripped so re-observations across chunks do not '
        'leak the sentinel', () {
      final buf =
          '$kMemoryProposalOpen{"memory_type":"identity"}'
          '$kMemoryProposalClose\n\nGot it.';
      final r = extractAndStripMemoryProposal(
        buffer: buf,
    ...
    test('sentinel that arrives without a trailing blank line is '
  ...
  group('ZK memory-proposal sentinel — SSE handler is wired', () {
    test('EVERY chat SSE loop calls extractAndStripMemoryProposal '
      ...
      while (true) {
        final decryptIdx =
            src.indexOf('buffer += decryptedChunk;', cursor);
    ...
    test('main.dart uses tryZkFinalizeMemoryProposal to POST '
    ...
    test('finalize best-effort helper swallows exceptions so no '
        'plaintext leaks into an error banner', () {
      final src = _readMain();
      final idx =
          src.indexOf('_finalizeMemoryProposalBestEffort({');
