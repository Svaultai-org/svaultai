...
void main() {
  ...
  group('ZK memory-proposal sentinel — SSE handler is wired', () {
    ...
    test('chat waits for confirmed memory persistence and surfaces failure',
        () {
      final src = _readMain();
      expect(
        src,
        contains('final saved = await _finalizeMemoryProposalBestEffort('),
      );
      expect(src, contains('memoryProposalFinalizeFailed = true'));
      expect(
        src,
        contains('I could not save that memory. Please try again.'),
      );
    });

    test('successful memory mutations force an immediate Memory page reload',
        () {
      final src = _readMain();
      expect(src, contains('int _memoryRefreshGeneration = 0'));
      expect(src, contains('_memoryRefreshGeneration++'));
      expect(src,
          contains("'memory-page-\$vaultName-\$_memoryRefreshGeneration'"));
    });
