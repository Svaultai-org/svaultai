...
//   * recovery kit generate → wrap → parse-seed → restore
...
import 'package:vault_ai_frontend/services/recovery_kit.dart';
...
void main() {
  group('recovery kit', () {
    test('generate → wrap → parse-seed → restore yields the same MVK',
        () async {
      final mvkBytes = Uint8List.fromList(
        List<int>.generate(32, (i) => (i * 7 + 3) & 0xFF),
      );
      final mvk = SecretKey(mvkBytes);

      final kit = await createRecoveryKitForMvk(mvk);
      expect(kit.seed.length, recoverySeedBytes);
      expect(kit.wrappedMvk.length, greaterThan(32));

      final humanSeed = kit.toHumanReadableSeed();
      expect(humanSeed.length, greaterThan(10));
      expect(humanSeed.contains('-'), isTrue);

      final reparsedSeed = parseHumanReadableSeed(humanSeed);
      expect(reparsedSeed, equals(kit.seed));
      ...
      expect(await restored.extractBytes(), equals(mvkBytes));
    }, timeout: const Timeout(Duration(minutes: 2)));

    test('wrong seed does not decrypt', () async {
      final mvkBytes = Uint8List.fromList(
        List<int>.generate(32, (i) => (i * 11 + 5) & 0xFF),
      );
      final kit = await createRecoveryKitForMvk(SecretKey(mvkBytes));
      final wrongSeed = Uint8List(recoverySeedBytes);
      expectLater(
        restoreMvkFromRecovery(
          seed: wrongSeed,
          wrappedMvk: kit.wrappedMvk,
          recoverySalt: kit.recoverySalt,
        ),
