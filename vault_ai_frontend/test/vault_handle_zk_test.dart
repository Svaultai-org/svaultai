...
void main() {
  group('vault_handle', () {
    ...
    test('fromDisplay tolerates dashes/case/whitespace variants', () {
      ...
      expect(
          vaultHandleFromDisplay(canonical.replaceAll('-', '')), equals(raw));
      expect(vaultHandleFromDisplay(' $canonical '), equals(raw));
      expect(
    ...
    test('fromDisplay rejects invalid input', () {
      expect(
          () => vaultHandleFromDisplay(''), throwsA(isA<InvalidVaultHandle>()));
      expect(
    ...
    test('confusable chars normalized (I=1, L=1, O=0, U=V)', () {
      final raw = generateVaultHandle();
      final disp = vaultHandleToDisplay(raw);
      final tail = disp.substring(vaultHandlePrefix.length).replaceAll('-', '');
      // Best-effort: only substitute if the tail actually contains 1/0
      // to substitute for.
      final withConfusables =
          tail.replaceFirst('1', 'I').replaceFirst('0', 'O');
    ...
    test('canonical account identifier ignores ASCII case and whitespace', () {
      final canonical = deriveVaultHandleFromUsername('Mixed Case Vault');
      expect(
        deriveVaultHandleFromUsername('  MIXED   case\tVault  '),
        canonical,
      );
      expect(
        deriveUsernameLookupV1('  MIXED   case\tVault  '),
        deriveUsernameLookupV1('mixed case vault'),
      );
    });

    test('Unicode vault names use stable UTF-8 lowercase derivation', () {
      expect(
        deriveVaultHandleFromUsername('ÄLICE 東京'),
        deriveVaultHandleFromUsername('älice 東京'),
      );
      expect(
        deriveUsernameLookupV1('ÄLICE 東京'),
        deriveUsernameLookupV1('älice 東京'),
      );
    });
