...
def requested_response_language(message: str) -> Optional[str]:
    if not isinstance(message, str):
        return None
    try:
        from vault_multilingual import detect_requested_language
        requested = detect_requested_language(message)
        if requested:
            return requested
    except Exception:
        pass
    match = _LANGUAGE_DIRECTIVE_RE.search(message)
    if match:
        return _LANGUAGES.get(match.group("language").lower())
    open_match = _OPEN_LANGUAGE_DIRECTIVE_RE.search(message)
    if not open_match:
        return None
    language_name = (
        open_match.group("language")
        or open_match.group("language_tail")
        or ""
    )
    return re.sub(r"\s+", " ", language_name).strip().lower() or None
...
def has_language_directive(message: str) -> bool:
    ...
    return bool(
        ...
        or requested_response_language(message)
    )
...
def _has_explicit_vault_intent(tokens: set[str]) -> bool:
    ...
    owned_collection = private_domain and {"i", "have"}.issubset(tokens)
    named_item_lookup = bool(tokens & {"show", "find", "open"}) and "me" in tokens
    return (
        (action and private_domain)
        or possessive_fact
        or owned_collection
        or named_item_lookup
    )
...
def _lightweight_response(
    underlying_intent: str,
    message: str,
    language: str,
) -> str:
    """Return an existing deterministic reply for obvious small talk only."""
    responses = _RESPONSES.get(language)
    if not responses or has_language_directive(message):
        return ""
    tokens = set(_tokens(message))
    key = None
    if underlying_intent == INTENT_CAPABILITY_QUESTION:
        allowed = (
            _CAPABILITY_CUES | _ASSISTANT_TARGETS |
            {"what", "are", "is", "me", "can", "could", "please", "kind", "of"}
        )
        if tokens <= allowed:
            key = "capability"
    elif underlying_intent == INTENT_ASSISTANT_IDENTITY:
        allowed = (
            _IDENTITY_CUES | _ASSISTANT_TARGETS |
            {"who", "are", "is", "me", "can", "could", "please", "kind", "of"}
        )
        if tokens <= allowed:
            key = "identity"
    elif underlying_intent == INTENT_GENERAL_CHAT:
        greeting_allowed = (
            _GREETING_WORDS | _SMALL_TALK_CUES | _ASSISTANT_TARGETS |
            {"how", "are", "is", "it", "me", "there", "the", "this"}
        )
        if tokens & _GREETING_WORDS and tokens <= greeting_allowed:
            key = "greeting"
        elif tokens & _THANKS_WORDS and len(tokens) <= 5:
            key = "thanks"
        elif tokens & _GOODBYE_WORDS and len(tokens) <= 5:
            key = "goodbye"
    return str(responses.get(key, "")) if key else ""
...
def route_general_chat(message: str, default_language: str = "en") -> Optional[GeneralChatRoute]:
    ...
    if underlying == INTENT_UNKNOWN_GENERAL:
        # Unknown prose without an explicit vault object/action is ordinary
        # conversation. Complexity, unfamiliar wording, and malformed text
        # are never evidence that retrieval is needed.
        if not (message or "").strip():
            return None
        if set(_tokens(message)) & (_RETRIEVAL_VERBS | _MUTATION_VERBS):
            return None
        underlying = INTENT_GENERAL_CHAT
    intent = INTENT_LANGUAGE_RESPONSE_REQUEST if open_language_request else underlying
    language = requested or default_language or "en"
    lightweight = _lightweight_response(underlying, message, language)
    if lightweight:
        return GeneralChatRoute(
            intent, underlying, language, lightweight,
            bool(requested), False,
        )
    # Natural conversation is generated by the configured model. This
    # router proves only that the turn is retrieval-free; using the finite
    # legacy response table here caused unsupported languages to silently
    # fall back to canned English.
    return GeneralChatRoute(
        intent, underlying, language, "",
        bool(requested), True,
    )
