...
_HANDLED_STAGE = va.STAGE_VIDEO_TRANSCRIPTION


def drain_video_transcription(
    *,
    vault_id: str,
    key: bytes,
    ...
    for _ in range(max_jobs):
        try:
            ...
            logger.exception(
                "[video-drain] claim_next_analysis_job failed "
                "vault=%s", vault_id,
            )
            break
        if job is None:
    ...
    *, job: dict, vault_id: str, key: bytes,
    ...
    if storage_mode == "inline":
        ...
        va.fail_analysis_job(
            job_id,
            error=(
                f"video transcription for chunked storage not "
    ...
    except vt.VideoTranscriptionError as exc:
        va.mark_file_analysis_failed(
    ...
    if not text:
        ...
        va.fail_analysis_job(
            job_id,
            error="video transcription returned no text",
            retry=False,
        )
        return False
    ...
    try:
        ...
        logger.exception(
            "[video] enqueue understanding failed file=%s", file_id,
        )
    ...
    try:
        ...
        logger.exception(
            "[video] enqueue content_chunking failed file=%s",
            file_id,
        )
    ...
    try:
        ...
        logger.exception(
            "[video] stale housekeeping failed file=%s", file_id,
        )
...
def _release_unhandled_job(job: dict) -> None:
    ...
    try:
        ...
        logger.exception(
            "[video-drain] release unhandled job failed job=%s "
            "stage=%s",
