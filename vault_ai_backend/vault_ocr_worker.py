    ...
    *,
    ...
    for _ in range(max_jobs):
        try:
            ...
            logger.exception(
                "[ocr-drain] claim_next_analysis_job failed vault=%s",
                vault_id,
            )
            break
        if job is None:
...
def _process_one_ocr_job(
    *, job: dict, vault_id: str, key: bytes,
    ...
    try:
        row = _load_file_row_for_ocr(vault_id=vault_id, file_id=file_id)
    ...
    if storage_mode == "inline":
        if not encrypted_blob:
            va.mark_file_analysis_failed(
                ...
                error="no encrypted bytes to OCR",
            )
            va.fail_analysis_job(
        ...
        va.fail_analysis_job(
            job_id,
            error=(
                f"OCR for chunked storage not implemented yet "
    ...
    try:
        ...
        logger.exception(
            "[ocr] enqueue understanding failed file=%s", file_id,
        )
    ...
    try:
        ...
        logger.exception(
            "[ocr] enqueue content_chunking failed file=%s", file_id,
        )
    ...
    try:
        ...
        logger.exception(
            "[ocr] stale housekeeping failed file=%s", file_id,
        )

    try:
        from vault_intelligence_updater import on_file_ocr
        on_file_ocr(vault_id)
    except Exception:
        pass
...
def _release_unhandled_job(job: dict) -> None:
    ...
    try:
        ...
        logger.exception(
            "[ocr-drain] release unhandled job failed job=%s stage=%s",
