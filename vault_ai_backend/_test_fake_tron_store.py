"""In-process fake for `crypto_tron_control_store`. Mirrors the
public surface exactly. Same fail-injection knobs (fail_register,
fail_consume, fail_lock, fail_pause_read) as the Solana/ETH fakes.
"""
...
class FakeTronStore:
    ...
    def seed_draft(
        ...
        *,
        ...
        network_id: str,
        sender_address: str,
        asset: str = "USDT_TRC20",
        destination_address: str = "",
        token_contract_address: str = "",
        amount_base_units: int = 0,
        fee_limit_sun: int = 100_000_000,
        raw_data_hex: str = "0a02" + "00" * 30,
        expiration_ms: int = 0,
        server_txid_hex: str = "aa" * 32,
        ttl_secs: int = 10 ** 8,
    ) -> None:
        now = _time.time()
        with self._lock:
            self._drafts[draft_id] = {
                ...
                "server_txid_hex": server_txid_hex,
                "created_at": now,
    ...
    def load_draft_readonly(
        ...
        with self._lock:
            ...
            return {
                ...
                "token_contract_address": d["token_contract_address"],
                "amount_base_units": int(d["amount_base_units"]),
                "fee_limit_sun": int(d["fee_limit_sun"]),
                "raw_data_hex": d["raw_data_hex"],
                "expiration_ms": int(d["expiration_ms"]),
                "server_txid_hex": d["server_txid_hex"],
                "consumed": d.get("consumed_at") is not None,
                "local_txid_hex": d.get("local_txid_hex"),
    ...
    def consume_claimed_draft(
        self, *, draft_id: str, claim_token: str, local_txid_hex: str,
    ) -> bool:
        if not claim_token or not local_txid_hex:
            return False
        now = _time.time()
        with self._lock:
            d = self._drafts.get(draft_id)
            ...
            if d.get("claim_token") != claim_token:
                return False
            d["consumed_at"] = now
            d["local_txid_hex"] = local_txid_hex
            return True
    ...
    def list_outgoing_history(
        ...
        with self._lock:
            ...
            for d in self._drafts.values():
                ...
                if d.get("network_id") != network_id:
                    continue
                if d.get("local_txid_hex") in (None, ""):
                    continue
                cands.append(d)
            cands.sort(
                key=lambda r: (
            ...
            for r in cands[:limit]:
                out.append({
                    ...
                    "fee_limit_sun": int(r["fee_limit_sun"]),
    ...
    def is_tron_send_paused(self) -> bool:
    ...
    def set_tron_send_paused(self, paused: bool) -> None:
        with self._lock:
...
_GLOBAL_FAKE: Optional[FakeTronStore] = None


def get_shared_fake() -> FakeTronStore:
    global _GLOBAL_FAKE
    if _GLOBAL_FAKE is None:
        _GLOBAL_FAKE = FakeTronStore()
    return _GLOBAL_FAKE
...
__all__ = ["FakeTronStore", "get_shared_fake", "reset_shared_fake"]
